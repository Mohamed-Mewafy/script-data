...
def clean_text(text):
    if not text:
        return ""
    # تنظيف أي أقواس أو علامات تنصيص أو رموز غير مطهوة
    text = re.sub(r'[\"\'\[\]\{\}]', '', text)
    return " ".join(text.split()).strip()

def extract_category_from_url_or_page(cat_url, page_genres, title):
    # محاولة استخراج تصنيف نظيف من رابط القسم الأساسي
...
def save_to_supabase(item_data, category_type, current_cat_url):
    ...
    try:
        # تحديد وتنظيف التصنيف بدقة خالية من الأقواس أو التنصيص
        ...
        if category_type == "movie":
            ...
            payload = {
                ...
                "category_type": clean_category,  # تخزين التصنيف النظيف هنا مباشرة
