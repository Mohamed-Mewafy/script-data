...
async def extract_media_details(page, media_url):
    ...
    def handle_request(request):
        ...
        if (
            any(
                ext in url
                for ext in [".mp4", ".m3u8", "downet", "video", "stream", "server"]
            )
            and "ionicons" not in url
...
def save_media(cat_name, data):
    try:
        if cat_name == "films":
            ...
            # بالنسبة للمسلسلات، هنعتبرها المسلسل الرئيسي لو مفيهاش تقسيم حلقات مفصل، أو هنفصلها كمسلسل
            series_payload = {
                "title": data["title"],
            ...
            # حفظ أو تحديث المسلسل في جدول tv_series
            res = (
                supabase.table("tv_series")
                .upsert(series_payload, on_conflict="page_url")
                .execute()
            )
            print(f"[☁️ Supabase - TV Series] Saved Series: {data['title']}")
